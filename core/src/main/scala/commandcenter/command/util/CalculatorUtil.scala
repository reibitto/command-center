...
final class CalculatorUtil(parameters: Parameters) {
  ...
  private def startsWithTerm[_: P]: P[BigDecimal] =
    P(for {
      value1 <- term
  ...
  private def exp[_: P]: P[BigDecimal] = P("exp" ~/ term).map(spire.math.exp)
  ...
  private def min[_: P]: P[BigDecimal] = P("min" ~/ multipleParametersParser(1)).map(_.min)

  private def random[_: P]: P[BigDecimal] =
  ...
  private def sinh[_: P]: P[BigDecimal] = P("sinh" ~/ term).map(spire.math.sinh(_))

  private def sqrt[_: P]: P[BigDecimal] = P("sqrt" ~/ term).map(spire.math.sqrt(_))

  private def tan[_: P]: P[BigDecimal] = P("tan" ~/ term).map(spire.math.tan(_))

  private def tanh[_: P]: P[BigDecimal] = P("tanh" ~/ term).map(spire.math.tanh(_))

  private def toDeg[_: P]: P[BigDecimal] = P("toDeg" ~/ term).map(_ / spire.math.pi * 180.0)

  private def toRad[_: P]: P[BigDecimal] = P("toRad" ~/ term).map(_ * spire.math.pi / 180.0)

  private def term[_: P]: P[BigDecimal] = P(number | const | NoCut(parens))

  private def number[_: P]: P[BigDecimal] =
    P(
  ...
  private def multipleParametersParser[_: P](count: Int): P[Seq[BigDecimal]] =
